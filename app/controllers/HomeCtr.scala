...
object DomainKey {
  ...
  val bcThucHienKhoiLuongQuy = "bcThucHienKhoiLuongquy"
  ...
  val congThucLuong = "congthucluong"
  val chungTuThanhToanLuong = "chungtuthanhtoanluong"
  val hesoluong = "hesoluong"
  val trangthainhanvien = "trangthainhanvien"
...
class HomeCtr(implicit val bindingModule: BindingModule) extends Controller with AuthenticationElement with AuthConfigImpl with MainTemplate with TransactionElement {
  ...
  val quanLyNhanVien = MenuItemDto(
    ...
    children = List(
      ...
      MenuItemDto(
        id = hesoluong,
        view = "sunerp.view.nhanvien.HeSoLuongList",
        text = "Hệ số lương",
        leaf = true
      ),
      MenuItemDto(
        id = trangthainhanvien,
        view = "sunerp.view.nhanvien.TrangThaiNhanVienList",
        text = "Trạng thái nhân viên",
        leaf = true
      ),
      MenuItemDto(
        id = diemHeSo,
  ...
  val baoCaoKhoiLuong = MenuItemDto(
    ...
    children = List(
      ...
      MenuItemDto(
        id = bcThucHienKhoiLuongQuy,
        view = "sunerp.view.report.BCThucHienKhoiLuongQuy",
        text = "BC thực hiện kl quý",
        leaf = true
      ),
      MenuItemDto(
      ...
      MenuItemDto(
        id = chungTuThanhToanLuong,
        view = "sunerp.view.report.ChungTuThanhToanLuong",
        text = "Chứng từ lương",
        leaf = true
      )
    )
  ...
  val setting = MenuItemDto(
    ...
    children = List(
      MenuItemDto(
        id = congThucLuong,
        view = "sunerp.view.setting.CongThucLuongList",
        text = "Công thức lương",
        leaf = true
      )
    )
  )
